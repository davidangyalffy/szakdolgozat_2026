...
OUTPUT_DIR    = PROJECT_ROOT / 'results' / 'index_analysis' / 'significance_tests_lstm'
...
def write_report(all_results: dict, path: Path):
    ...
    w('Index cikkek: 2019 vs 2021  |  HVG–Origo LSTM modellek')
    w(f'Szignifikancia-szint: α = {ALPHA}')
    w(sep)

    for model_year, res in all_results.items():
        w(f'\n{dash}')
        w(f'MODELL: {model_year}-es LSTM pipeline')
        w(dash)
...
def create_plot(all_results: dict, probas: dict):
    ...
    for ax, model_year in zip(axes, MODEL_YEARS):
        ...
        ax.set_title(f'{model_year}-es LSTM modell\n'
    ...
    plt.suptitle('Index cikkek P(Origo) eloszlása — 2019 vs 2021  (LSTM)',
...
def main():
    ...
    for model_year in MODEL_YEARS:
        ...
        proba  = model.predict(padded, batch_size=BATCH_SIZE, verbose=0).flatten()
