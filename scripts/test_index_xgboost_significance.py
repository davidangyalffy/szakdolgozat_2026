...
using the trained HVG–Origo XGBoost models.

For each available model year:
...
MODEL_BASE    = PROJECT_ROOT / 'results' / 'xgboost_results'
INDEX_DATA    = PROJECT_ROOT / 'processed_data' / 'final' / 'index_tfidf_lemmatized.json'
OUTPUT_DIR    = PROJECT_ROOT / 'results' / 'index_analysis' / 'significance_tests_xgboost'

MODEL_YEARS   = [2019, 2021]   # extend to [2019, 2021] once the 2021 model is trained
ALPHA         = 0.05
...
def write_report(all_results: dict, path: Path):
    ...
    w('Index cikkek: 2019 vs 2021  |  HVG–Origo XGBoost modellek')
    w(f'Szignifikancia-szint: α = {ALPHA}')
    w(sep)

    for model_year, res in all_results.items():
        w(f'\n{dash}')
        w(f'MODELL: {model_year}-es XGBoost pipeline')
        w(dash)
...
def create_plot(all_results: dict, probas: dict):
    ...
    for ax, model_year in zip(axes, MODEL_YEARS):
        ...
        ax.set_title(f'{model_year}-es XGBoost modell\n'
    ...
    plt.suptitle('Index cikkek P(Origo) eloszlása — 2019 vs 2021  (XGBoost)',
...
def main():
    ...
    print('\n' + '=' * 70)
    print('INDEX CIKKEK SZIGNIFIKANCIA-VIZSGÁLATA (XGBoost)')
    print('=' * 70)
    ...
    for model_year in MODEL_YEARS:
        print(f'\n{"─"*55}')
        print(f'  {model_year}-es XGBoost betöltése …')

        vec_path   = MODEL_BASE / str(model_year) / f'vectorizer_{model_year}.pkl'
        model_path = MODEL_BASE / str(model_year) / f'xgboost_model_{model_year}.pkl'
        ...
        print('  Vektorizálás + előrejelzés …')
